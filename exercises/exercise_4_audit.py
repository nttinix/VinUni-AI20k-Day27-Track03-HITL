...
Implementation checklist: the audit() body, one AuditEntry per node for
analyze/route/commit/auto_approve/synthesize, and two each for human_approval
and escalate (before + after the interrupt).
"""
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision="reject" if action == "rejected" else "approve",
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"High confidence auto path: {action}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="approve",
        reason=answer_summary or "Reviewer supplied escalation answers",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
